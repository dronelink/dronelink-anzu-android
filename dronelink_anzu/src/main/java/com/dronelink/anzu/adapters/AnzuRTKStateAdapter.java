...
public class AnzuRTKStateAdapter implements RTKStateAdapter, RTKSystemStateListener, INetworkServiceInfoListener {
    ...
    public AnzuRTKStateAdapter(final Context context, final AnzuDroneAdapter drone) {
    ...
    public RTKReferenceStationSource getReferenceStationSource() {
        return systemState == null ? null : DronelinkAnzu.getRTKReferenceStationSource(systemState.getRtkReferenceStationSource());
    }

    @Override
    public RTKServiceState getServiceState() {
        return DronelinkAnzu.getRTKServiceState(serviceState);
    }
    ...
    public List<Message> getStatusMessages() {
        ...
        Message message = customNetworkError == null ? null : DronelinkAnzu.getMessage(context, customNetworkError);
        if (message != null) {
        ...
        if (getServiceState() == RTKServiceState.TRANSMITTING) {
            message = DronelinkAnzu.getMessage(context, positioningSolution);
            if (message != null) {
                messages.add(message);
            }

            if (location == null) {
                messages.add(new Message(context.getString(R.string.AnzuRTKStateAdapter_custom_network), context.getString(R.string.AnzuRTKStateAdapter_custom_network_location_unavailable), Message.Level.ERROR));
            }
        }

        message = serviceState == null ? null : DronelinkAnzu.getMessage(context, serviceState);
        if (message != null) {
            messages.add(message);
        }

        message = systemState == null ? null : DronelinkAnzu.getMessage(context, systemState.getError());
        if (message != null) {
            messages.add(message);
        }
        ...
        if (getServiceState() == RTKServiceState.TRANSMITTING) {
            if (location != null) {
                messages.add(new Message(context.getString(R.string.AnzuRTKStateAdapter_custom_network_location), String.format("%.6f", location.getLatitude()) + ", " + String.format("%.6f", location.getLongitude()), Message.Level.INFO));
            }
        }
